...
/** Immutable descriptor seam for same-frame ReSTIR DI candidate generation. */
public final class RtDirectCandidatePipeline {
    ...
    private RtDirectCandidatePipeline(RtContext ctx, long descriptorSetLayout, long descriptorPool,
    ...
    public static RtDirectCandidatePipeline create(
            RtContext ctx,
        ...
        if (reservoirSlots.length != SLOT_COUNT) {
            throw new IllegalArgumentException("Candidate pipeline requires exactly two reservoir slots");
        }
        VkDevice vk = ctx.vk();
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            check(VK10.vkCreateDescriptorSetLayout(vk, dslInfo, null, handle),
                    "vkCreateDescriptorSetLayout(direct candidates)");
            ...
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, descriptorSetLayout,
                    "direct candidates descriptor set layout");
            ...
            check(VK10.vkAllocateDescriptorSets(vk, allocateInfo, setsBuffer),
                    "vkAllocateDescriptorSets(direct candidates)");
            ...
            check(VK10.vkCreatePipelineLayout(vk, layoutInfo, null, handle),
                    "vkCreatePipelineLayout(direct candidates)");
            long pipelineLayout = handle.get(0);
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_PIPELINE_LAYOUT, pipelineLayout,
                    "direct candidates pipeline layout");

            long module = loadModule(vk, stack, "direct_candidates.comp.spv");
            ...
            return new RtDirectCandidatePipeline(ctx, descriptorSetLayout, descriptorPool,
    ...
    public void dispatch(VkCommandBuffer cmd, int writeSlot, int width, int height,
        ...
        try (MemoryStack stack = MemoryStack.stackPush();
             RtDebugLabels.Scope ignored = RtDebugLabels.scope(ctx, cmd, "direct candidates")) {
    ...
    private static void bindResources(
        ...
        VkDescriptorBufferInfo.Buffer bufferInfos = VkDescriptorBufferInfo.calloc(SLOT_COUNT, stack);
        VkWriteDescriptorSet.Buffer writes =
                VkWriteDescriptorSet.calloc(SLOT_COUNT * (IMAGE_COUNT + 1), stack);
        for (int slot = 0; slot < SLOT_COUNT; slot++) {
            for (int binding = 0; binding < IMAGE_COUNT; binding++) {
                int imageIndex = slot * IMAGE_COUNT + binding;
                int writeIndex = slot * (IMAGE_COUNT + 1) + binding;
            ...
            bufferInfos.get(slot).buffer(reservoirSlots[slot].handle)
                    .offset(0L).range(reservoirSlots[slot].size);
            writes.get(slot * (IMAGE_COUNT + 1) + IMAGE_COUNT)
                    .sType$Default().dstSet(sets[slot]).dstBinding(IMAGE_COUNT)
                    .descriptorCount(1).descriptorType(VK10.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER)
                    .pBufferInfo(VkDescriptorBufferInfo.create(bufferInfos.address(slot), 1));
        }
    ...
    private static long loadModule(VkDevice vk, MemoryStack stack, String name) {
        byte[] bytes;
        try (InputStream in = RtDirectCandidatePipeline.class.getResourceAsStream(SHADER_DIR + name)) {
            if (in == null) {
