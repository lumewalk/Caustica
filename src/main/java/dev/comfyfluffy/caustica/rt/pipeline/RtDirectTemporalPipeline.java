...
/** Immutable current/previous descriptor pairs for temporal direct-reservoir reuse. */
public final class RtDirectTemporalPipeline {
    ...
    private RtDirectTemporalPipeline(RtContext ctx, long descriptorSetLayout, long descriptorPool,
    ...
    public static RtDirectTemporalPipeline create(
            RtContext ctx,
            long receiverPositionMaterialView,
            long receiverNormalRoughnessView,
            long receiverAlbedoSssView,
            long receiverMotionView,
            long validationMetadataView,
            long debugColorView,
            RtBuffer[] reservoirSlots) {
        if (reservoirSlots.length != SLOT_COUNT) {
            throw new IllegalArgumentException("Temporal reuse requires exactly two reservoir slots");
        }
        VkDevice vk = ctx.vk();
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            check(VK10.vkCreateDescriptorSetLayout(vk, dslInfo, null, handle),
                    "vkCreateDescriptorSetLayout(direct temporal)");
            ...
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, descriptorSetLayout,
                    "direct temporal descriptor set layout");
            ...
            check(VK10.vkAllocateDescriptorSets(vk, allocateInfo, setsBuffer),
                    "vkAllocateDescriptorSets(direct temporal)");
            ...
            check(VK10.vkCreatePipelineLayout(vk, layoutInfo, null, handle),
                    "vkCreatePipelineLayout(direct temporal)");
            long pipelineLayout = handle.get(0);
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_PIPELINE_LAYOUT, pipelineLayout,
                    "direct temporal pipeline layout");

            long module = loadModule(vk, stack, "direct_temporal.comp.spv");
            ...
            check(VK10.vkCreateComputePipelines(vk, VK10.VK_NULL_HANDLE, pipelineInfo, null, handle),
                    "vkCreateComputePipelines(direct temporal)");
            long pipeline = handle.get(0);
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_PIPELINE, pipeline,
                    "direct temporal compute pipeline");
            VK10.vkDestroyShaderModule(vk, module, null);

            bindResources(vk, stack, descriptorSets,
                    receiverPositionMaterialView, receiverNormalRoughnessView,
                    receiverAlbedoSssView, receiverMotionView, validationMetadataView,
                    debugColorView, reservoirSlots);
            return new RtDirectTemporalPipeline(ctx, descriptorSetLayout, descriptorPool,
                    descriptorSets, pipelineLayout, pipeline);
        }
    }

    public void dispatch(VkCommandBuffer cmd, int writeSlot, int width, int height,
                         ByteBuffer pushConstants) {
        if (writeSlot < 0 || writeSlot >= SLOT_COUNT) {
            throw new IllegalArgumentException("Reservoir write slot out of range: " + writeSlot);
        }
        try (MemoryStack stack = MemoryStack.stackPush();
             RtDebugLabels.Scope ignored = RtDebugLabels.scope(ctx, cmd, "direct temporal reuse")) {
            VK10.vkCmdBindPipeline(cmd, VK10.VK_PIPELINE_BIND_POINT_COMPUTE, pipeline);
            VK10.vkCmdBindDescriptorSets(cmd, VK10.VK_PIPELINE_BIND_POINT_COMPUTE, pipelineLayout, 0,
                    stack.longs(descriptorSets[writeSlot]), null);
    ...
    private static void bindResources(
            ...
            long receiverAlbedoSssView, long receiverMotionView, long validationMetadataView,
            long debugColorView, RtBuffer[] reservoirSlots) {
        long[] imageViews = {
                receiverPositionMaterialView,
                receiverNormalRoughnessView,
                receiverAlbedoSssView,
                receiverMotionView,
                validationMetadataView,
                debugColorView
        };
        ...
        for (int slot = 0; slot < SLOT_COUNT; slot++) {
            for (int binding = 0; binding < IMAGE_COUNT; binding++) {
                int imageIndex = slot * IMAGE_COUNT + binding;
                int writeIndex = slot * writesPerSet + binding;
                imageInfos.get(imageIndex).imageView(imageViews[binding])
                        .imageLayout(VK10.VK_IMAGE_LAYOUT_GENERAL);
                writes.get(writeIndex).sType$Default().dstSet(sets[slot]).dstBinding(binding)
            ...
            int previousSlot = 1 - slot;
            long[] buffers = {reservoirSlots[slot].handle, reservoirSlots[previousSlot].handle};
            long[] ranges = {reservoirSlots[slot].size, reservoirSlots[previousSlot].size};
            for (int buffer = 0; buffer < BUFFER_COUNT; buffer++) {
                int bufferIndex = slot * BUFFER_COUNT + buffer;
                int binding = IMAGE_COUNT + buffer;
                int writeIndex = slot * writesPerSet + binding;
                bufferInfos.get(bufferIndex).buffer(buffers[buffer]).offset(0L).range(ranges[buffer]);
                writes.get(writeIndex).sType$Default().dstSet(sets[slot]).dstBinding(binding)
    ...
    private static long loadModule(VkDevice vk, MemoryStack stack, String name) {
        byte[] bytes;
        try (InputStream in = RtDirectTemporalPipeline.class.getResourceAsStream(SHADER_DIR + name)) {
            if (in == null) {
